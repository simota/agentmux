...
impl DaemonRuntime {
    ...
    /// Broadcast a raw input-action sequence to every agent resolved by
    /// `target`, writing it verbatim into each idle target PTY (the
    /// "Commands panel" / tmux `synchronize-panes` equivalent).
    ///
    /// Returns `(delivered, skipped)`:
    /// - `delivered`: agents whose PTY received the actions.
    /// - `skipped`: agents that resolved to the target but were *not* written
    ///   to because a human was typing into that pane within the
    ///   `human_input_quiet` window. This is the spec invariant "never
    ///   auto-inject into a pane where a human is currently typing" — broadcast
    ///   input is an automated write (not a per-pane human keystroke), so it
    ///   obeys the same quiet guard that `deliver_idle_messages_for_agent` uses.
    ///
    /// An unresolvable target (no matching agents) propagates the bus
    /// `UserError` to the caller unchanged.
    ///
    /// Unlike `send_input_script`, this path deliberately does NOT call
    /// `record_human_input_for_agent`: broadcast input is generated by the
    /// operator's Commands panel, not by typing directly into a specific pane,
    /// so marking each target pane as "human just typed" would wrongly suppress
    /// the bus auto-injection quiet window for those panes. This mirrors the
    /// automated message-injection path (`write_prepared_message_injection`),
    /// which also writes to PTYs without recording human input.
    pub async fn broadcast_input(
        &self,
        target: &MessageTarget,
        actions: &[InputAction],
    ) -> Result<BroadcastInputOutcome> {
        let now = DateTimeUtc::now_utc();

        // Resolve targets and partition into deliver/skip under a single read
        // lock so the quiet-window decision is consistent across the batch.
        // An empty resolution surfaces the bus UserError unchanged.
        let (deliverable, skipped) = {
            let state = self.state.read().await;
            let recipients = state.messages.resolve_target(target)?;
            let quiet_period = state.injection_timing.human_input_quiet;
            ...
            for agent_id in recipients {
                match state.agents.get(&agent_id) {
                    ...
                    _ => deliverable.push(agent_id),
                }
            }
            (deliverable, skipped)
        };

        // Write sequentially, matching the single-injection lock/await order
        // (each target takes the short-lived state.read() + PTY mutex inside
        // write_input_actions_to_agent_pty). Concurrency is intentionally not
        // increased beyond the existing single-injection pattern.
...
#[derive(Debug, Clone)]
pub struct BroadcastInputOutcome {
    pub delivered: Vec<AgentSessionId>,
    pub skipped: Vec<AgentSessionId>,
}

/// Build the input script used to broadcast a raw action sequence to one target
/// PTY. The actions are written verbatim (the caller already appended any
/// trailing PressEnter), so this preserves the operator's exact keystrokes.
pub(crate) fn broadcast_input_script(
    agent_id: &AgentSessionId,
    actions: &[InputAction],
) -> InputScript {
    InputScript {
        ...
        reason: "agent.broadcast_input".to_string(),
