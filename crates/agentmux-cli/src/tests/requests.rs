...
fn bare_spawn_response_requires_agent_id() {
    let error = agent_id_from_spawn_response(DaemonResponse::ok("req_bare_agent_spawn", json!({})))
        .unwrap_err();
...
fn agent_set_role_request_carries_agent_id_and_role() {
    let request = agent_set_role_request("agent_001".to_string(), "qa-lead".to_string()).unwrap();
