...
fn message_requests_target_daemon_ipc() {
    ...
    let send =
        message_send_request("agent_01HX".to_string(), "hello".to_string(), None, None).unwrap();
...
fn send_commands_default_to_inject_and_accept_override_flags() {
    let cli = Cli::try_parse_from(["agentmux", "agent", "send", "agent_01HX", "hello"]).unwrap();
...
fn message_send_defaults_kind_handoff_and_priority_normal() {
    let request =
        message_send_request("role:tester".to_string(), "body".to_string(), None, None).unwrap();
...
fn message_send_attributes_agent_when_env_present() {
    ...
    let request = message_send_request("role:tester".to_string(), "body".to_string(), None, None);
...
fn message_send_rejects_empty_body_before_ipc() {
    let error =
        message_send_request("agent_01HX".to_string(), "  ".to_string(), None, None).unwrap_err();
